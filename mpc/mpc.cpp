...
typedef union {
  ...
  mpc_pdata_not_t not_;
  mpc_pdata_repeat_t repeat;
  mpc_pdata_and_t and_;
  mpc_pdata_or_t or_;
} mpc_pdata_t;
...
int mpc_parse_input(mpc_input_t *i, mpc_parser_t *init, mpc_result_t *final) {
  ...
  while (!mpc_stack_empty(stk)) {
    ...
    switch (p->type) {
      ...
      case MPC_TYPE_NOT:
        if (st == 0) { mpc_input_mark(i); MPC_CONTINUE(1, p->data.not_.x); }
        if (st == 1) {
          if (mpc_stack_popr(stk, &r)) {
            mpc_input_rewind(i);
            p->data.not_.dx(r.output);
            MPC_FAILURE(mpc_err_new(i->filename, i->state, "opposite", mpc_input_peekc(i)));
          } else {
            mpc_input_unmark(i);
            mpc_stack_err(stk, r.error);
            MPC_SUCCESS(p->data.not_.lf());
          }
        }
      
      case MPC_TYPE_MAYBE:
        if (st == 0) { MPC_CONTINUE(1, p->data.not_.x); }
        if (st == 1) {
          if (mpc_stack_popr(stk, &r)) {
            MPC_SUCCESS(r.output);
          } else {
      ...
      case MPC_TYPE_OR:
        ...
        if (p->data.or_.n == 0) { MPC_SUCCESS(NULL); }
        
        if (st == 0) { MPC_CONTINUE(st+1, p->data.or_.xs[st]); }
        if (st <= p->data.or_.n) {
          if (mpc_stack_peekr(stk, &r)) {
            mpc_stack_popr(stk, &r);
          ...
          if (st <  p->data.or_.n) { MPC_CONTINUE(st+1, p->data.or_.xs[st]); }
          if (st == p->data.or_.n) { MPC_FAILURE(mpc_stack_merger_err(stk, p->data.or_.n)); }
        }
      
      case MPC_TYPE_AND:
        
        if (p->data.and_.n == 0) { MPC_SUCCESS(p->data.and_.f(0, NULL)); }
        
        if (st == 0) { mpc_input_mark(i); MPC_CONTINUE(st+1, p->data.and_.xs[st]); }
        if (st <= p->data.and_.n) {
          if (!mpc_stack_peekr(stk, &r)) {
            mpc_input_rewind(i);
            mpc_stack_popr(stk, &r);
            mpc_stack_popr_out(stk, st-1, p->data.and_.dxs);
            MPC_FAILURE(r.error);
          }
          if (st <  p->data.and_.n) { MPC_CONTINUE(st+1, p->data.and_.xs[st]); }
          if (st == p->data.and_.n) { mpc_input_unmark(i); MPC_SUCCESS(mpc_stack_merger_out(stk, p->data.and_.n, p->data.and_.f)); }
        }
...
static void mpc_undefine_or(mpc_parser_t *p) {
  ...
  int i;
  for (i = 0; i < p->data.or_.n; i++) {
    mpc_undefine_unretained(p->data.or_.xs[i], 0);
  }
  free(p->data.or_.xs);
...
static void mpc_undefine_and(mpc_parser_t *p) {
  ...
  int i;
  for (i = 0; i < p->data.and_.n; i++) {
    mpc_undefine_unretained(p->data.and_.xs[i], 0);
  }
  free(p->data.and_.xs);
  free(p->data.and_.dxs);
...
static void mpc_undefine_unretained(mpc_parser_t *p, int force) {
  ...
  switch (p->type) {
    ...
    case MPC_TYPE_MAYBE:
    case MPC_TYPE_NOT:
      mpc_undefine_unretained(p->data.not_.x, 0);
      break;
...
mpc_parser_t *mpc_or(int n, ...) {
  ...
  p->type = MPC_TYPE_OR;
  p->data.or_.n = n;
  p->data.or_.xs = (mpc_parser_t**)malloc(sizeof(mpc_parser_t*) * n);
  
  va_start(va, n);  
  for (i = 0; i < n; i++) {
    p->data.or_.xs[i] = va_arg(va, mpc_parser_t*);
  }
  va_end(va);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  p->data.and_.n = n;
  p->data.and_.f = f;
  p->data.and_.xs = (mpc_parser_t**)malloc(sizeof(mpc_parser_t*) * n);
  p->data.and_.dxs = (mpc_dtor_t*)malloc(sizeof(mpc_dtor_t) * (n-1));

  va_start(va, f);  
  for (i = 0; i < n; i++) {
    p->data.and_.xs[i] = va_arg(va, mpc_parser_t*);
  }
  for (i = 0; i < (n-1); i++) {
    p->data.and_.dxs[i] = va_arg(va, mpc_dtor_t);
  }  
  va_end(va);
...
mpc_val_t *mpcf_ctor_str(void) { return (mpc_val_t*)calloc(1, 1); }
...
static void mpc_print_unretained(mpc_parser_t *p, int force) {
  ...
  if (p->type == MPC_TYPE_NOT)   { mpc_print_unretained(p->data.not_.x, 0); printf("!"); }
  if (p->type == MPC_TYPE_MAYBE) { mpc_print_unretained(p->data.not_.x, 0); printf("?"); }
  ...
  if (p->type == MPC_TYPE_OR) {
    printf("(");
    for(i = 0; i < p->data.or_.n-1; i++) {
      mpc_print_unretained(p->data.or_.xs[i], 0);
      printf(" | ");
    }
    mpc_print_unretained(p->data.or_.xs[p->data.or_.n-1], 0);
    printf(")");
  }
  
  if (p->type == MPC_TYPE_AND) {
    printf("(");
    for(i = 0; i < p->data.and_.n-1; i++) {
      mpc_print_unretained(p->data.and_.xs[i], 0);
      printf(" ");
    }
    mpc_print_unretained(p->data.and_.xs[p->data.and_.n-1], 0);
    printf(")");
  }
...
mpc_parser_t *mpca_and(int n, ...) {
  ...
  for (i = 0; i < (n-1); i++) {
    p->data.and_.dxs[i] = (mpc_dtor_t)mpc_ast_delete;
  }    
  va_end(va);
